/*
  ...
  1. New Tables
    - `exercises`
      ...
      - `equipamento` (text, equipment needed)
      - `series` (integer, number of sets)
      - `repeticoes` (text, repetitions)
      - `descanso` (text, rest time)
      - `nivel` (text, difficulty level)
      - `objetivos` (text, training objectives)
      - `observacoes` (text, notes)
      - `gif_url` (text, animation URL)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Security
    - Enable RLS on `exercises` table
    - Add policies for authenticated and anonymous users to read exercises

  3. Data
    - Insert comprehensive exercise database with different muscle groups and difficulty levels
*/
...
-- Create policies for reading exercises
...
-- BRAÇOS - Iniciante
('Rosca Direta', 'Bíceps', 'Halteres', 3, '10-12', '60s', 'iniciante', 'ganho_massa, definicao', 'Cotovelos fixos, movimento apenas do antebraço', 'https://example.com/rosca-direta.gif'),
('Tríceps Testa', 'Tríceps', 'Halteres', 3, '10-12', '60s', 'iniciante', 'ganho_massa, definicao', 'Cotovelos fixos, desça até a testa', 'https://example.com/triceps-testa.gif'),
('Rosca Martelo', 'Bíceps', 'Halteres', 3, '10-12', '45s', 'iniciante', 'ganho_massa, definicao', 'Pegada neutra, movimento controlado', 'https://example.com/rosca-martelo.gif'),

-- BRAÇOS - Intermediário
('Rosca Barra', 'Bíceps', 'Barra', 3, '8-10', '90s', 'intermediario', 'ganho_massa, definicao', 'Pegada na largura dos ombros, movimento completo', 'https://example.com/rosca-barra.gif'),
('Tríceps Pulley', 'Tríceps', 'Cabo', 3, '10-12', '45s', 'intermediario', 'ganho_massa, definicao', 'Cotovelos colados ao corpo, extensão completa', 'https://example.com/triceps-pulley.gif'),
('Rosca Concentrada', 'Bíceps', 'Halteres', 3, '10-12', '45s', 'intermediario', 'definicao, ganho_massa', 'Apoie o cotovelo na coxa, movimento isolado', 'https://example.com/rosca-concentrada.gif'),

-- BRAÇOS - Avançado
('Rosca 21', 'Bíceps', 'Barra', 3, '21', '90s', 'avancado', 'ganho_massa, definicao', '7 reps parciais baixo, 7 alto, 7 completas', 'https://example.com/rosca-21.gif'),
('Tríceps Francês', 'Tríceps', 'Barra', 3, '8-10', '90s', 'avancado', 'ganho_massa, definicao', 'Deitado, desça a barra atrás da cabeça', 'https://example.com/triceps-frances.gif'),
...
-- GLÚTEOS - Iniciante
('Agachamento Sumo', 'Glúteos', 'Peso Corporal', 3, '12-15', '60s', 'iniciante', 'definicao, ganho_massa', 'Pés afastados, pontas viradas para fora', 'https://example.com/agachamento-sumo.gif'),
('Ponte de Glúteo', 'Glúteos', 'Peso Corporal', 3, '15-20', '45s', 'iniciante', 'definicao, ganho_massa', 'Deitado, eleve o quadril contraindo o glúteo', 'https://example.com/ponte-gluteo.gif'),
('Elevação de Quadril', 'Glúteos', 'Peso Corporal', 3, '12-15', '45s', 'iniciante', 'definicao, ganho_massa', 'De quatro, eleve a perna para trás', 'https://example.com/elevacao-quadril.gif'),

-- GLÚTEOS - Intermediário
('Agachamento Búlgaro', 'Glúteos', 'Halteres', 3, '10-12', '60s', 'intermediario', 'definicao, ganho_massa', 'Pé traseiro elevado, foque no glúteo', 'https://example.com/agachamento-bulgaro-gluteo.gif'),
('Stiff Unilateral', 'Glúteos', 'Halteres', 3, '10-12', '60s', 'intermediario', 'ganho_massa, definicao', 'Uma perna só, foque no alongamento', 'https://example.com/stiff-unilateral.gif'),
('Hip Thrust', 'Glúteos', 'Barra', 3, '12-15', '90s', 'intermediario', 'ganho_massa, definicao', 'Costas no banco, eleve o quadril com peso', 'https://example.com/hip-thrust.gif'),

-- GLÚTEOS - Avançado
('Hip Thrust com Barra', 'Glúteos', 'Barra', 4, '8-12', '120s', 'avancado', 'ganho_massa, definicao', 'Movimento explosivo, contração máxima', 'https://example.com/hip-thrust-barra.gif'),
('Passada Lateral', 'Glúteos', 'Halteres', 3, '10-12', '60s', 'avancado', 'definicao, ganho_massa', 'Passo lateral amplo, retorne à posição inicial', 'https://example.com/passada-lateral.gif'),
...
CREATE INDEX idx_exercises_equipamento ON exercises(equipamento);
CREATE INDEX idx_exercises_objective ON exercises USING gin(to_tsvector('portuguese', objetivos));